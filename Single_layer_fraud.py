...
# idea - make both training methods start from the same weights
import numpy as np
...
print("Output layer shape:", ow.shape)
...
global ow
global hw
hw = np.random.uniform(-1, 1, (30, HIDDEN_SIZE))
ow = np.random.uniform(-1, 1, (HIDDEN_SIZE, 2))
    ...
    for i in range(weights.shape[0]):
        for j in range(weights.shape[1]):
            ...
            if j == 0:
              ...
              resolution = int(-1*resolution_sf*cur_loss+15)
              #print(f"cur_loss is {cur_loss}")
...
# =========================
mlp_g = MLP(dropout_rate=DROPOUT_RATE)
...
##################################################################################
# ---- Grover pre-training metrics (epoch 0) ----
_, y_train_pred0 = mlp_g.forward(x_train, training=False)
train_ce0 = -np.mean(np.sum(y_train_1hot * np.log(y_train_pred0 + 1e-9), axis=1))
train_reg0 = WEIGHT_DECAY * (np.sum(mlp_g.weights_1**2) + np.sum(mlp_g.weights_2**2))
grover_train_loss_all.append(train_ce0 + train_reg0)

_, y_test_pred0 = mlp_g.forward(x_test, training=False)
test_ce0 = -np.mean(np.sum(y_test_1hot * np.log(y_test_pred0 + 1e-9), axis=1))
test_reg0 = WEIGHT_DECAY * (np.sum(mlp_g.weights_1**2) + np.sum(mlp_g.weights_2**2))
grover_test_loss_all.append(test_ce0 + test_reg0)
############################################
...
def adam_baseline_train_losses(num_epochs, hidden_size, lr=1e-2, beta1=0.9, beta2=0.999, eps=1e-8):
    ...
    print(f"current tl is {tr_losses}, current tl is {te_losses}")
...
print(f"all training accuracy {grover_train_acc_all}")
print(f"all test accuray {grover_test_acc_all}")

# =========================
# Figure-2 style plot (FIXED)
# =========================

epochs_axis = np.arange(0, NUM_EPOCHS + 1)  # <-- includes pre-training epoch 0
...
plt.xlabel("Epoch (0 = pre-training)")
plt.ylabel("Loss (cross-entropy + L2)")
plt.title("Grover vs ADAM — identical initial weights verified")
plt.legend()
plt.grid(True)
plt.show()
