...
# KEEP THIS SMALL — Grover explodes otherwise
HIDDEN_SIZE_1 = 256
HIDDEN_SIZE_2 =256
...
W1_INIT = np.random.uniform(-1, 1, (30, HIDDEN_SIZE_1))
W2_INIT = np.random.uniform(-1, 1, (HIDDEN_SIZE_1, HIDDEN_SIZE_2))
W3_INIT = np.random.uniform(-1, 1, (HIDDEN_SIZE_2, 2))  # BINARY OUTPUT
...
# Balance the dataset (fraud vs legit)
fraud = df[df["Class"] == 1]
legit = df[df["Class"] == 0].sample(len(fraud), random_state=42)

# Before converting to numpy, remove duplicates
df_small = pd.concat([fraud, legit]).sample(frac=1, random_state=42)
df_small = df_small.drop_duplicates(subset=df_small.columns.difference(['Class']))

# Then continue with your previous code
...
print(grover_train_loss[-1])
print(grover_test_loss[-1])
